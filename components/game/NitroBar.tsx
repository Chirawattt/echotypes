...
export default function NitroBar({ energy, maxEnergy, isLowEnergy, heatLevel}: NitroBarProps) {
    ...
    const barGradient = heatLevel?.bgColor || 'from-blue-500 via-blue-400 to-blue-500';
    const bgColor = isLowEnergy ? 'bg-red-500/20 text-red-300' : 
                    (heatLevel?.level && heatLevel.level > 2) ? 'bg-red-500/20 text-red-300' :
                    'bg-blue-500/20 text-blue-300';

    // Low energy warning sound effect
    useEffect(() => {
        if (isLowEnergy && energy > 0) {
            // Start beeping sound - faster beeping as energy gets lower
            const beepInterval = Math.max(100, 500 - (3 - energy) * 100); // Much faster beeping (100-500ms)
            ...
            intervalRef.current = setInterval(() => {
                if (audioRef.current) {
                    ...
                    audioRef.current.play().catch(() => {
                        // Handle audio play promise rejection
                    });
                }
            }, beepInterval);
        ...
        <div className="w-full max-w-2xl mx-auto mb-4">
            ...
            <div className="flex justify-between items-center mb-2">
                ...
                <motion.div
                    ...
                    animate={isLowEnergy || (heatLevel?.level && heatLevel.level > 2) ? {
                    ...
                    transition={{ 
                        backgroundColor: { duration: 0.3, repeat: (isLowEnergy || (heatLevel?.level && heatLevel.level > 2)) ? Infinity : 0 },
                        scale: { 
                            duration: 0.3, 
                            repeat: (isLowEnergy || (heatLevel?.level && heatLevel.level > 2)) ? Infinity : 0,
                            repeatType: "reverse"
                        }
                    }}
                >
            ...
            <div className="relative">
                ...
                <div className="w-full h-6 sm:h-8 bg-gray-800/50 rounded-full border-2 border-gray-600/50 overflow-hidden">
                    ...
                    <motion.div
                        ...
                        animate={{ 
                            ...
                            opacity: (isLowEnergy || (heatLevel?.effects.hasPulse)) ? [0.6, 1, 0.6] : 1,
                        ...
                        transition={{ 
                            ...
                            opacity: { 
                                ...
                                repeat: (isLowEnergy || heatLevel?.effects.hasPulse) ? Infinity : 0 
                            },
                            scale: {
                            ...
                            boxShadow: { 
                                duration: (isLowEnergy || heatLevel?.level && heatLevel.level > 2) ? 0.4 : 1,
                                repeat: (isLowEnergy || heatLevel?.level && heatLevel.level > 2) ? Infinity : 0 
                            }
                        }}
                    >
                ...
                <div className="absolute inset-0 flex justify-between items-center px-1">
                    {[...Array(10)].map((_, i) => (
                        <div
                            key={i}
                            className="w-0.5 h-2 sm:h-3 bg-gray-500/30"
                        />
                    ))}
                </div>
            </div>
            ...
            {isLowEnergy && (
                ...
                >
                    <span className="text-red-400 font-bold text-sm sm:text-base" style={{ fontFamily: "'Caveat Brush', cursive" }}>
                        ⚠️ CRITICAL ENERGY LEVEL! ⚠️
                    </span>
                </motion.div>
            )}

            {/* Hidden audio element for beeping sound */}
            <audio
                ref={audioRef}
                preload="auto"
            >
